...
class MaskNet6(nn.Module):
    ...
    def forward(self, target_image, ref_imgs):
        ...
        input = [target_image]
        input.extend(ref_imgs)#t,t-1,t+1
        input = torch.cat(input, 1)#list 2 tensor
        out_conv1 = self.conv1(input)#[4,15,128,416]->[4,16,64,208]
        out_conv2 = self.conv2(out_conv1)#[]->[4,32,32,104]
        out_conv3 = self.conv3(out_conv2)#[]->[4,64,16,52]
        out_conv4 = self.conv4(out_conv3)#[]->[4,128,8,26]
        out_conv5 = self.conv5(out_conv4)#[]->[4,256,4,13]
        out_conv6 = self.conv6(out_conv5)#[]->[4,256,2,7]
